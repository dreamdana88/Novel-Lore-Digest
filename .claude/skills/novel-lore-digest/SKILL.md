...
This Claude Code skill supports analysis of long novels, story collections, series fiction, case-based fiction, adventure arcs, fantasy worldbuilding, folklore horror, urban fantasy, and supernatural investigation stories.

Use it when the user wants to transform source fiction into reusable lore documents:
- character profiles
- relationship maps
- worldbuilding rules
- timelines
...
Before any analysis or generation, read `PROJECT_CONFIG.md`.
...
- Do not modify `source_raw/`.
- Do not invent information unsupported by source text.
- Do not skip local notes and jump directly to final character cards or worldbook entries.
- Do not turn a one-off story rule into a global setting rule without evidence.
- Do not deeply analyze one-time background characters unless `PROJECT_CONFIG.md` asks for it.
- Mark uncertain information as 【待核查】.
...
Every important conclusion must include a source file, story title, chapter title, or local-note reference.
